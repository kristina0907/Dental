...
import MovingMaterialsTable from "./MovingMaterialsTable";
...
const headCells = [
    ...
    { id: 'quantity', numeric: false, label: 'Кол-во' },
...
function MaterialsMoving({materials,selectsCabinet,itemsQuantity}) {
    ...
    const [getIdStorage, setIdStorage] = React.useState();
    ...
    useEffect(() => {
        if(getUpdated && getIdStorage && category){
            store.dispatch(getQuantity({
                "storage_id":getIdStorage.value,
    ...
    const changeBranchStorage = (event) => {
        setIdStorage({
            "key":event.target[event.target.selectedIndex].id,
            ...
            <Grid item xs={12} md={9} lg={9} xl={10}>
                ...
                <Grid item xs={12}>
                    ...
                    <div className="add_list">
                        <Link className="item_add_list active" to={"/materials/add/moving"}>Перемещение</Link>
                        <Link className="item_add_list" to={"/materials/add/waybill"}>Накладную</Link>
                    </div>
                    <div>
                                            ...
                                            Object.keys(materials).map((item) => {
                                                ...
                                                return (
                                                    <Grid item xs={12} sm={2}>
                                                        <div className={`item_category_price ${category && category === material.id ? 'active' : ''}`} onClick={() => setCategoryActive(material.id)}>
                                        ...
                                        <div className="table_filter">
                                            <div>
                                                <select required name={`branch_storage_id`}
                                                        ...
                                                        onChange={changeBranchStorage}
                                                >
                                                    <option value="">Со склада</option>

                                                    {selectsCabinet && selectsCabinet.length  ?
                                                        (selectsCabinet).map((cabinet,key) => {
                                                            return (
                                                                <option key={key} id={key} value={cabinet.id}>{cabinet.name}</option>
                                                            )
                                                        })
                                                        :null}
                                                </select>
                                                    ...
                                                    (
                                                        <>
                                                            ...
                                                            (itemsQuantity && itemsQuantity.length)?
                                                                Object.keys(itemsQuantity).map((item) => {
                                                                    let material=itemsQuantity[item].material;
                                                                    itemsQuantity[item].quantity!== 0 &&
                                                                    rows.push({
                                                                        "id": material.id,
                                                                        "name": material.name,
                                                                        "unit":material.unit,
                                                                        "quantity": itemsQuantity[item].quantity,
                    ...
                    <Grid item className={`contaoner-info`}>
                        ...
                        {!hideBtnClicked.block_4? (
                            <>
                                ...
                                <MovingMaterialsTable updated={setUpdated} moving={getmovingArray} IdStorage={getIdStorage} deleteAll={deleteAllMaterial} deleteMaterial={deleteMovingMaterial}/>
                            </>
                        ) : (
                            <div className={`show-shedule-btn`}>
                                <div className={`row`}>
...
const mapStateToProps = (state) => ({
    ...
    itemsQuantity:state.materials.itemsQuantity
});

export default connect(mapStateToProps, { getAllMaterials,deleteCategory, deleteMaterial, getQuantity })(MaterialsMoving);
