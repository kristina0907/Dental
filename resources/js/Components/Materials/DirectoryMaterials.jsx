...
import EditCategoryMaterial from "../helpers/EditCategoryMaterial";
...
import AddNewCategoryMaterial from "../helpers/AddNewCategoryMaterial";
import AddNewMaterial from "../helpers/AddNewMaterial"
import Search from "../helpers/Search";
import Pagination from "@material-ui/lab/Pagination";
import Header from "../helpers/Header";
import HeaderRight from "../helpers/HeaderRight";
import IncomingCall from "../IncomingCall";
import {getBranches} from "../../actions/settingsActions";
...
function MoreIcon({index, row,editMaterial, deliteMaterial}) {
    return(
            ...
            {(popupState) => (
                <React.Fragment>
                    ...
                    <Menu className={"MoMenu"}{...bindMenu(popupState)}>
                        <MenuItem onClick={() => editMaterial({"material":row})}>Изменить</MenuItem>
                        <MenuItem onClick={() => deliteMaterial(row.id)}>Удалить</MenuItem>
                    </Menu>
                </React.Fragment>
            )}
        </PopupState>
...
function Materials({materials,units}) {
    ...
    const [getVisibleAddMaterial, setVisibleAddMaterial] = useState(false);
    ...
    const ondeliteMaterial = (id) => {
        store.dispatch(deleteMaterial(id));
        setUpdated(true);
        handleClose();
    }
    ...
    const [getInvoices, setInvoices] = useState(false);
    const rows = [];
                        ...
                        {!hideBtnClicked.block_3? (
                            <div>
                                ...
                                <form action="">
                                    ...
                                    <select name="" id="">
                                        <option value="1">Штрихкод</option>
                                    </select>
                            ...
                            {!hideBtnClicked.block_1? (
                                <React.Fragment>
                                    ...
                                    <div>
                                        ...
                                        <AddNewCategoryMaterial updatedCategory={setUpdated} getVisible={getVisibleAddCategory} visible={setVisibleAddCategory}/>
                                    </div>
                                    <Grid container>
                                        {materials ? (
                                                        ...
                                                        <div className={`item_category_price ${category && category === item ? 'active' : ''}`} onClick={() => setCategoryActive(item)}>
                                                            {category && category === item ?
                                                                <div className="main_category_icon">
                                                                    ...
                                                                    <EditCategoryMaterial updatedCategory={setUpdated} getVisible={getVisibleEditMainCategory} category={material}  visible={setVisibleEditMainCategory}/>
                                                                    <IconButton aria-haspopup="true"  onClick={() => ondeleteCategory(material.id)} edge="end">
                                                                        <DeleteIcon />
                                                                    </IconButton>
                                                                </div>
                                                                : null}
                                ...
                                {!hideBtnClicked.block_2? (
                                    <React.Fragment>
                                        ...
                                        <Grid container className={"search_materials"}>
                                            ...
                                            <Grid item xs={12} sm={6} className={"right_btn_materials"}>
                                                <button className="btn_new_category" type="button" onClick={() => setVisibleAddMaterial(true)}>
                                                    Добавить
                                                </button>
                                            </Grid>
                                        </Grid>
                                        <AddNewMaterial units={units && units.length ? units : null } materialCategory={materials[category].id} updateMaterial={setUpdated} getVisible={getVisibleAddMaterial} visible={setVisibleAddMaterial}/>

                                        { (category !== undefined?
                                            <React.Fragment>
                                                {getVisibleEditProduct &&
                                                <EditMaterial units={units && units.length ? units : null } data={editProductData} updateMaterial={setUpdated} visible={setVisibleEditProduct} />
                                                }
                                                {
                                                                            ...
                                                                            <TableCell align="right">
                                                                                <div className="branch-more">
                                                                                    <FormControlLabel
                                                                                        ...
                                                                                        control={<MoreIcon index={index} deliteMaterial={ondeliteMaterial} editMaterial={handleEditMaterial} row={row} />}
                                                                                    />
                                                                                </div>
                                                                            </TableCell>
                                                                        </TableRow>
...
const mapStateToProps = (state) => ({
    ...
    units:state.materials.units
});

export default connect(mapStateToProps, { getAllMaterials,deleteCategory, deleteMaterial })(Materials);
...
const userHistoryRowStyles = makeStyles({
    root: {
        '& td, & th': {
            borderBottom: "none",
        },
