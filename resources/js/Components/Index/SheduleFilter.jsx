...
function ScheduleFilter({user, filter, filterData, currentDate}) {
            ...
            {!hideBtnClicked.filter_1? (
                <div>
                    ...
                    <form onSubmit={handleSubmit(filterSubmit)}>
                        <input type="hidden" ref={register} name={"current_date"} value={moment(currentDate).format('DD-MM-YYYY')}/>
                        ...
                        {filter && !filter.data.professions.length?
                            <Skeleton width={"100%"} height={"40px"} animation={"wave"}/>
                            :
                            <select multiple={true} ref={register} name="profession_id" defaultValue={currentProfession} onChange={(e) => setCurrentProfession(parseInt(e.target.value))}>
...
export default connect(mapStateToProps, {getDoctorsScheduleFilter})(ScheduleFilter);
